...
----------------------------------
Unit tests for Milestone 8 (dataset.py) acceptance criteria.

Acceptance Criteria:
  - QuantileTransformer produces mean ≈ 0 for normalized continuous features.
  - Dataset correctly returns tensors of expected shape.
  - DataLoader yields (x_cont, x_cat, y) with the right dtypes.
  - WeightedRandomSampler fires without error.
  - encode_categoricals maps Protocol and TCP Flags correctly.
"""
...
def sample_df() -> pd.DataFrame:
    """Create a small synthetic DataFrame matching the CIC-DDoS2019 schema."""
    ...
    return pd.DataFrame(
        {
            ...
            "Protocol": rng.choice([0, 6, 17], size=n),  # HOPOPT, TCP, UDP
...
class TestEncodeCategoricals:
    ...
    def test_tcp_flags_clamped_to_0_63(self, sample_df: pd.DataFrame) -> None:
        ...
        df = sample_df.copy()
        df["TCP Flags"] = 127  # Out of range → should be clamped to 63
        encoded = encode_categoricals(df)
...
class TestQuantileTransformer:
    ...
    def test_scaler_serialization(self, tmp_path: Path, sample_df: pd.DataFrame) -> None:
        """Scaler must be serializable and loadable without loss."""
...
class TestNetworkFlowDataset:
    ...
    def test_string_labels_binarized(self, tmp_path: Path, sample_df: pd.DataFrame) -> None:
        """String labels like 'BENIGN'/'DDoS' must be binarized to 0/1."""
        from src.fl_client.dataset import NetworkFlowDataset
...
class TestBuildDataloaders:
    ...
    def test_train_val_split_sizes(self, sample_csv: Path) -> None:
        ...
        assert abs((n_train + n_val) - 1000) <= 1  # Off-by-one tolerance
