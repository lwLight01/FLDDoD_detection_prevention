...
class TestGitignore:
    ...
    def test_ignores_model_checkpoints(self):
        ...
        assert (
            "*.pt" in content or "checkpoints/" in content
        ), ".gitignore must exclude model checkpoint files"

    def test_ignores_raw_data(self):
        content = (PROJECT_ROOT / ".gitignore").read_text()
        assert "data/raw/*" in content, ".gitignore must exclude raw data files (large datasets)"

    def test_ignores_certificates(self):
        content = (PROJECT_ROOT / ".gitignore").read_text()
        assert "*.pem" in content or "certs/" in content, ".gitignore must exclude TLS certificates"

    def test_ignores_coverage_artifacts(self):
        content = (PROJECT_ROOT / ".gitignore").read_text()
        assert (
            ".coverage" in content or "htmlcov/" in content
...
class TestCIWorkflow:
    ...
    def test_ci_triggers_on_push(self):
        ...
        assert "push" in triggers, "CI must trigger on push events"

    def test_ci_triggers_on_pull_request(self):
        data = self._load_ci_yaml()
        triggers = data.get(True, data.get("on", {}))
        assert "pull_request" in triggers, "CI must trigger on pull_request events"

    def test_ci_has_lint_and_test_job(self):
        data = self._load_ci_yaml()
        jobs = data.get("jobs", {})
        assert "lint-and-test" in jobs, "CI must define a 'lint-and-test' job"

    def test_ci_has_docker_build_job(self):
        data = self._load_ci_yaml()
        jobs = data.get("jobs", {})
        assert "docker-build" in jobs, "CI must define a 'docker-build' job"

    def test_ci_has_integration_tests_job(self):
        data = self._load_ci_yaml()
        jobs = data.get("jobs", {})
        assert "integration-tests" in jobs, "CI must define an 'integration-tests' job"

    def test_lint_job_runs_black(self):
        data = self._load_ci_yaml()
        steps = data["jobs"]["lint-and-test"]["steps"]
        step_texts = [str(s) for s in steps]
        assert any("black" in s for s in step_texts), "CI lint job must run black formatter check"

    def test_lint_job_runs_flake8(self):
        data = self._load_ci_yaml()
        steps = data["jobs"]["lint-and-test"]["steps"]
        step_texts = [str(s) for s in steps]
        assert any("flake8" in s for s in step_texts), "CI lint job must run flake8 linter"
    ...
    def test_lint_job_runs_isort(self):
        ...
        step_texts = [str(s) for s in steps]
        assert any(
            "isort" in s for s in step_texts
    ...
    def test_lint_job_runs_pytest(self):
        ...
        steps = data["jobs"]["lint-and-test"]["steps"]
        step_texts = [str(s) for s in steps]
        assert any("pytest" in s for s in step_texts), "CI lint job must run pytest"

    def test_integration_job_has_postgres_service(self):
        data = self._load_ci_yaml()
        services = data["jobs"]["integration-tests"].get("services", {})
        assert "db" in services, "Integration job must define a PostgreSQL service"
        assert "timescale" in services["db"]["image"], "CI DB service must use TimescaleDB image"
