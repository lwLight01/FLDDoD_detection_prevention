...
class TestAlembicMigration:
    ...
    def _migration_content(self) -> str:
        ...
        assert len(migration_files) == 1, \
            "Exactly one initial schema migration file must exist"
        return migration_files[0].read_text(encoding="utf-8")

    def test_migration_file_exists(self):
        versions_dir = PROJECT_ROOT / "alembic" / "versions"
        assert any(versions_dir.glob("*initial_schema*.py")), \
    ...
    def test_migration_creates_all_9_tables(self):
        ...
        expected_tables = [
            "roles", "users", "fl_rounds", "fl_clients",
            "fl_client_updates", "models", "traffic_history",
            "attack_alerts", "mitigation_actions",
        ]
        for table in expected_tables:
            assert table in content, \
                f"Migration must reference table '{table}'"

    def test_migration_enables_timescaledb_extension(self):
        content = self._migration_content()
        assert "timescaledb" in content.lower(), \
            "Migration must enable the timescaledb extension"

    def test_migration_enables_uuid_ossp_extension(self):
        content = self._migration_content()
        assert "uuid-ossp" in content, \
            "Migration must enable the uuid-ossp extension"

    def test_migration_creates_hypertable_for_traffic_history(self):
        content = self._migration_content()
        assert "create_hypertable" in content and "traffic_history" in content, \
